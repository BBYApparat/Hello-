...
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `gang`, `gang_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`'

if Config.SaltyInventory then
	loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `gang`, `gang_grade`, `group`, `position`, `inventory`, `skin`, `metadata`, `user_licenses`'
end
...
if not Config.Multichar then
	...
	AddEventHandler('playerConnecting', function(_, _, defferals)
		defferals.defer()
		local playerId = source
		local identifier = ESX.GetIdentifier(playerId)
		if not Core.Whitelist[identifier] then
			local card = '{"type":"AdaptiveCard","$schema":"http://adaptivecards.io/schemas/adaptive-card.json","version":"1.4","body":[{"type":"Image","url":"","horizontalAlignment":"Center"},{"type":"Container","items":[{"type":"TextBlock","text":"Second Life","wrap":true,"fontType":"Default","size":"ExtraLarge","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"This server has a whitelisted community. You are not in whitelist. Join our discord to get whitelisted in order to play.","wrap":true,"size":"Large","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Click to join Second Life Discord Server","style":"destructive","iconUrl":"","url":" /second-life"}],"horizontalAlignment":"Center"}],"style":"default","bleed":true,"height":"stretch"},{"type":"Image","url":"https://cdn.discordapp.com/attachments/1271066329691979917/1273227297830469684/banner.png?ex=66bf2a53&is=66bdd8d3&hm=8948a28ae816c9cc56c363613bd46d32679622b98f0c625189ba326262af98e4&","selectAction":{"type":"Action.OpenUrl","url":""},"horizontalAlignment":"Center"}]}'
			local toEnd = nil
			local count = 0
			local options = {
				{ name = "Player",  value = GetPlayerName(playerId),   inline = true },
				{ name = "ID",      value = identifier, inline = false },
			}
			local foundDiscord = nil
			...
			if foundDiscord then
				foundDiscord = string.gsub(foundDiscord, "discord:", "")
			...
			ESX.DiscordLogFields("Whitelist", "Non Whitelisted user tried to join!", "red", options)
			while not toEnd do
				Wait(1000)
				defferals.presentCard(card, function(data, rawData)
			...
			return defferals.done('You are not in our whitelist. Join our discord for more!')
		else
			return defferals.done()
		end
	end)
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	if not Config.SaltyInventory then
		userData.loadout = {}
	end
	local result = MySQL.prepare.await(loadPlayer, { identifier })
	local job, grade, jobObject, gradeObject = result.job, tostring(result.job_grade)
	local gang, gang_grade, gangObject, gang_gradeObject = result.gang, tostring(result.gang_grade)
	...
	if ESX.DoesGangExist(gang, gang_grade) then
		gangObject, gang_gradeObject = ESX.Gangs[gang], ESX.Gangs[gang].grades[gang_grade]
	else
		print(('[^3WARNING^7] Ignoring invalid gang for ^5%s^7 [gang: ^5%s^7, grade: ^5%s^7]'):format(identifier, gang, gang_grade))
		gang, grade = 'nogang', '0'
		gangObject, gang_gradeObject = ESX.Gangs[gang], ESX.Gangs[gang].grades[gang_grade]
	end
	...
	userData.gang.grade = tonumber(gang_grade)
	userData.gang.grade_name = gang_gradeObject.name
	userData.gang.grade_label = gang_gradeObject.label

	-- Inventory
	if Config.OxInventory or Config.SaltyInventory then
	...
	-- Licenses
	if result.user_licenses then
		userData.licenses = json.decode(result.user_licenses)
	end
	...
	local xPlayer
	if Config.SaltyInventory then
		xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.gang, userData.playerName, userData.coords, userData.metadata, userData.licenses)
	else
		xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.gang, userData.loadout, userData.playerName, userData.coords, userData.metadata) 
	end
	ESX.Players[playerId] = xPlayer
	GlobalState.PlayerCount = #ESX.GetPlayers()
	Core.playersByIdentifier[identifier] = xPlayer

	if userData.firstname then
		xPlayer.set('firstName', userData.firstname or '')
		xPlayer.set('lastName', userData.lastname or '')
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		TriggerEvent('esx:playerDropped', xPlayer.source, reason, xPlayer)
		TriggerClientEvent('esx:DrawLeaveReason', -1, xPlayer.source, reason, xPlayer.coords, xPlayer.identifier)
		Core.playersByIdentifier[xPlayer.identifier] = nil
		Wait(1000)
		Core.RemovePlayerFromJobList(xPlayer)
...
AddEventHandler('esx:playerLogout', function(playerId, cb)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	if xPlayer then
		TriggerEvent('esx:playerDropped', playerId, 'Logout', xPlayer)
		TriggerClientEvent('esx:DrawLeaveReason', -1, playerId, reason, xPlayer.coords, xPlayer.identifier)
		Core.playersByIdentifier[xPlayer.identifier] = nil
		Core.RemovePlayerFromJobList(xPlayer)
		Core.SavePlayer(xPlayer, function()
...
AddEventHandler("esx:setJob", function(playerId, newJob, lastJob)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	Core.RemovePlayerFromJobList(xPlayer, lastJob)
	Core.AddPlayerToJobList(xPlayer)
	print(json.encode(Core.jobPlayers))
end)

local TimedOutUses = {}

if Config.SaltyInventory then
	local canUseItem = function(plyId)
		if TimedOutUses[tostring(plyId)] then
			return false
		end
		return true
	end

	AddEventHandler('esx:resetUsage', function(id)
		Wait(1000)
		if TimedOutUses[id] then
			TimedOutUses[id] = nil
		end
	end)

	RegisterServerEvent("esx:UseItem", function(item)
		local src = source
		local xPlayer = ESX.GetPlayerFromId(src)
		local xItem = xPlayer.GetItemByName(item.name)
		if item ~= nil and item.amount > 0 and xItem and xItem.amount > 0 then
			if ESX.CanUseItem(item.name) and canUseItem(xPlayer.source) then
				TimedOutUses[tostring(src)] = true
				ESX.UseItem(src, item)
				if not ESX.Config.BlacklistedNotificationItems[item.name] then
					xPlayer.ItemBox(item.name, "use")
				end
				TriggerEvent('esx:resetUsage', tostring(src))
			end
		end
	end)

	RegisterServerEvent("esx:removeItem", function(itemName, amount, slot)
		local item = ESX.Shared.Items[itemName]
		if amount < 1 or not item then
			return
		end
		local src = source
		local xPlayer = ESX.GetPlayerFromId(src)
		if xPlayer then
			local hasItem = xPlayer.GetItemByName(itemName)
			if hasItem and hasItem.amount >= amount then
				xPlayer.removeInventoryItem(itemName, amount, slot)
				xPlayer.ItemBox(itemName, "remove", amount)
			end
		end
	end)

	AddEventHandler("esx:onAddInventoryItem", function(playerId, name, amount)
		if name == "phone" then
			TriggerClientEvent('lb-phone:itemAdded', playerId)
		end
	end)
	
	AddEventHandler("esx:onRemoveInventoryItem", function(playerId, name, amount)
		if name == "phone" then
			TriggerClientEvent('lb-phone:itemRemoved', playerId)
		end
	end)
end
