...
RegisterNetEvent('esx:playerLoaded', function(xPlayer, isNew, skin)
    ...
    if Config.Multichar then
		...
		TriggerEvent("CanX-SpawnSelector:Open", source)
    end
	-- DisableNPCDrops
...
if Config.SaltyInventory then
	RegisterNetEvent('esx:setWeaponAmmo', function(slot, amount)
		if ESX.PlayerData.inventory[slot] then
			if ESX.PlayerData.inventory[slot].info and ESX.PlayerData.inventory[slot].info.ammo then
				ESX.PlayerData.inventory[slot].info.ammo = amount
			end
		end
		TriggerEvent('esx:setInventory', ESX.PlayerData.inventory)
	end)
	
	RegisterNetEvent('setWeaponAmmo', function(slot, amount)
		ESX.PlayerData.inventory[slot].info.ammo = amount
		TriggerEvent('esx:setInventory', ESX.PlayerData.inventory)
	end)
	
	RegisterNetEvent('esx:updateItemInfo', function(slot, data)
		...
		TriggerEvent('esx:setInventory', inv)
	end)

	RegisterNetEvent('esx:setInventory', function(inventory)
...
RegisterNetEvent('esx:DrawLeaveReason', function(id, reason, coords, identifier)
    local run = true
    CreateThread(function()
        while run do
            ESX.Game.Utils.DrawText3D(coords, 'ID: ~b~'..id..'\nIdentifier: ~y~'..identifier..'\nReason: ~r~'..reason, 0.8)
            Wait(0)
        end
    end)
    Wait(60000)
...
RegisterNetEvent('inventory:notification', function(sender, msg, time, type)
	if sender then
...
RegisterNetEvent('esx:deleteVehicle', function(radius)
    if radius and tonumber(radius) then
        ...
        local vehicles = ESX.Game.GetVehiclesInArea(GetEntityCoords(PlayerPedId()), radius)
