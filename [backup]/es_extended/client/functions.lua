    ...
    for i = 1, itemCount do
        ...
        for _, item in pairs(ESX.PlayerData.inventory) do
            if item.name == itemName then
                if count then
                    returnData[itemName] = returnData[itemName] + item.amount
                else
...
function ESX.Progressbar(message, length, Options)
    if GetResourceState("progressbar") ~= "missing" then
        return exports["progressbar"]:Progress(name, duration)
    end
    print("[^1ERROR^7] ^5ESX Progressbar^7 is Missing!")
end

ESX.ShowProgressBar = function(msg, time, data, onFinish, onCancel)
...
ESX.StopProgressBar = function()
    if exports['ox_lib']:progressActive() then
        exports['ox_lib']:cancelProgress()
    end
end

ESX.ShowNotification = function(msg, timer, type)
    local message = msg
    local mType = 'inform'
    if string.find(message,'~r~') or type == "error" then
        mType = 'error'
    elseif string.find(message,'~y~') or type == "warning" then
        mType = 'warning'
    elseif string.find(message,'~g~') or type == "success" then
        mType = 'success'
    elseif string.find(message,'~b~') or type == "inform" then
        mType = 'inform'
    end
    while string.find(message,'~r~') 
        or string.find(message,'~g~')
        or string.find(message,'~y~') 
        or string.find(message,'~c~') 
        or string.find(message,'~b~') 
        or string.find(message,'~w~') 
        or string.find(message,'~m~') 
        or string.find(message,'~n~') 
        or string.find(message,'~o~') 
        or string.find(message,'~p~') 
        or string.find(message,'~u~') 
        or string.find(message,'~s~') do
        Wait(0)
        ...
        message = message:gsub('%~o~', '')
        message = message:gsub('%~p~', '')
        message = message:gsub('%~u~', '')
        message = message:gsub('%~s~', '')
    end
    exports['ox_lib']:notify({
        type = mType,
        description = message,
        duration = 5000 or timer,
        position = 'top',
    })
end
...
ESX.ShowHelpNotification = function(msg, thisFrame, beep, duration)
    ...
    if lastHelp ~= message then
        ...
        exports['ox_lib']:showTextUI(message, {position = 'left-center'})
        CreateThread(function()
            while (GetGameTimer() - lastTime) < 1000 do
                Wait(1000)
            end
...
ESX.Game.GetVehicleProperties = function(vehicle)
    ...
    return {
        ...
        custom_tire         = GetResourceState('esx_lscustoms') == 'started' and exports['esx_lscustoms']:GetVehicleTireType(vehicle) or 0,
        custom_turbo        = GetResourceState('esx_lscustoms') == 'started' and exports['esx_lscustoms']:GetVehicleTurbo(vehicle) or 0,
        custom_engine       = GetResourceState('esx_lscustoms') == 'started' and  exports['esx_lscustoms']:GetVehicleEngine(vehicle) or 0,
        ...
        fuelLevel           = Entity(vehicle).state.fuel or GetVehicleFuelLevel(vehicle) or 100,
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
    ...
    if props.fuelLevel ~= nil then
        SetVehicleFuelLevel(vehicle, tonumber(props.fuelLevel))
        Entity(vehicle).state.fuel = tonumber(props.fuelLevel)
    end
    ...
    if props.drift_tire ~= nil and GetGameBuildNumber() >= 2372 then 
        SetDriftTyresEnabled(vehicle, props.drift_tire or false) 
    end
    if props.custom_tire and GetResourceState('esx_lscustoms') == 'started' then
        exports['esx_lscustoms']:SetVehicleTireType(vehicle, props.custom_tire) 
    end
    if props.custom_turbo and GetResourceState('esx_lscustoms') == 'started' then 
        exports['esx_lscustoms']:SetVehicleTurbo(vehicle, props.custom_turbo) 
    end
    if props.custom_engine and GetResourceState('esx_lscustoms') == 'started' then 
        exports['esx_lscustoms']:SetVehicleEngine(vehicle, props.custom_engine) 
    end
...
ESX.GetItem = function(name)
	...
	for k,v in pairs(inventory) do
		if v.name and tostring(v.name) == tostring(name) then
			itemData = ESX.Shared.Items[v.name]
			itemData.amount = v.amount
			itemData.slot = v.slot
			itemData.info = v.info or ""
			return itemData
		end
	end
    return itemData
