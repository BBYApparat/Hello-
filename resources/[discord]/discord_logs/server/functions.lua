...
if webhooksFile == nil then
    print('^5[discord_logs] ^1Error: Could not load webhooks file. Make sure you didn\'t make a typo.^0')
...
function GetPlayerDetails(src, config, channel)
    ...
    if GetConvar("steam_webApiKey", "false") ~= 'false' then
        ...
        _steamID = ""
        _steamURL = ""
        TriggerEvent('Prefech:discord_logs:errorLog', 'You need to set a steam api key in your server.cfg for the steam identifiers to work!.')
    end
    ...
    if config.Session or config.PlayTime and not webhooksFile[channel].Hide['PlayTime'] then
        if GetResourceState('Prefech_playTime') == "started" then
            ...
            --TriggerEvent('Prefech:discord_logs:errorLog', 'Prefech_playTime is not installed.')
            _total = ""
            _session = ""
        end
    else
    ...
    if config['useESX'] and channel ~= 'joins' then
        ...
        if xPlayer then
            ...
            if xPlayer.identifier then
                _esx = _esx.."\n**Charachter ID:** "..xPlayer.identifier
            end
...
function CreateLog(args)
    ...
    if webhooksFile[args.channel] then
        ...
        if args.player_id then
            ...
            Player_Details = GetPlayerDetails(args.player_id, configFile, args.channel, args)
        ...
        if args.player_2_id then
            ...
            Player_2_Details = GetPlayerDetails(args.player_2_id, configFile, args.channel, args)
            message['embeds'][1].fields[2]  = {
        ...
        if args.ss then
            if not args.ss.single then
                message['embeds'][1].image = {
                    ["url"] = args.ss[args.player_2_id].url
                }
                message['embeds'][1].thumbnail = {
                    ["url"] = args.ss[args.player_id].url
                }
            else
                message['embeds'][1].image = {
                    ["url"] = args.ss[args.player_id].url
        ...
        print("discord_logs: Webhooks channel not found. ("..args.channel..")")
    end
end

function getStatus()
	if status == 404 or status == 401 and webhooksFile[channel].webhook ~= "DISCORD_WEBHOOK" and webhooksFile[channel].webhook ~= "" then
		print('^3Warn: discord_logs webhook. Possible invalid webhook for "'..channel..'" webhook. Status code: '..status)
	end
end
