...
if lang == nil then
    print('^5[discord_logs] ^1Error: Could not load language file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

if cfgFile == nil then
    print('^5[discord_logs] ^1Error: Could not load config file. Make sure you didn\'t make a typo.^0')
    return StopResource(GetCurrentResourceName())
end

RegisterNetEvent('Prefech:discord_logs:Debug')
AddEventHandler('Prefech:discord_logs:Debug', log)

RegisterNetEvent('Prefech:discord_logs:errorLog')
AddEventHandler('Prefech:discord_logs:errorLog', errorLog)

function debugLog(x)
	if JD_Debug then
		print("^5[discord_logs]^0 " .. x)
	end
end

function errorLog(x)
	print("^5[discord_logs]^1 " .. x .."^0")
end
...
RegisterNetEvent('discord_logs:add')
AddEventHandler('discord_logs:add', function(msg, player_1, player_2, channel, url)
	args ={
		['EmbedMessage'] = msg,
		['color'] = color,
		['channel'] = channel
	...
	if url then
		args.responseUrl = url
	end
	ServerFunc.CreateLog(args)
end)

exports('discord', function(msg, player_1, player_2, channel, url)
	args ={
		['EmbedMessage'] = msg,
		['color'] = color,
		['channel'] = channel
			...
			PerformHttpRequest('https://cdn.prefech.dev/api/checkBan.php', function(err, text, headers)
				if text == nil then
					...
					if text ~= 'Safe' then
						...
						return deferrals.done("\nPrefech | Global Banned.\nReason: "..one.."\nUUID: "..two.."\nTo appeal this ban please join our discord:  /6rcWxBzKAG")
					else
...
AddEventHandler("playerJoining", function(source, oldID)
	local nameFile = LoadResourceFile(GetCurrentResourceName(), "./json/names.json")
	local loadedFile = json.decode(nameFile)
    local ids = ExtractIdentifiers(source)
...
AddEventHandler('esx:playerDropped', function(playerId, reason)
	ServerFunc.CreateLog({EmbedMessage = lang['DefaultLogs'].Left:format(GetPlayerName(playerId), reason), player_id = playerId, channel = 'leaving'})
end)
...
RegisterNetEvent('Prefech:DeliverSS', function(img, rId)
	RID[rId].url = img
	RID[rId].waiting = false
end)

RegisterServerEvent('Prefech:playerDied')
AddEventHandler('Prefech:playerDied',function(args)
	local source = source
	local uniqueID = math.random(111111, 99999999)
	local uniqueID2 = math.random(111111, 99999999)
	if args.type == 2 then
		RID[uniqueID] = {
			waiting = true
		}
		...
		local webhooksFile = Webhooks
		TriggerClientEvent('Prefech:RequestSS', args.player_2_id, webhooksFile['imageStore'].webhook, uniqueID)
		TriggerClientEvent('Prefech:RequestSS', source, webhooksFile['imageStore'].webhook, uniqueID2)

		while RID[uniqueID].waiting do Citizen.Wait(1000) end
		while RID[uniqueID2].waiting do Citizen.Wait(1000) end
	else
		RID[uniqueID] = {
			waiting = true
		}
		local webhooksFile = Webhooks
		TriggerClientEvent('Prefech:RequestSS', source, webhooksFile['imageStore'].webhook, uniqueID)
		while RID[uniqueID].waiting do Citizen.Wait(1000) end
	end
...
if GetCurrentResourceName() ~= "discord_logs" then
    errorLog('This recource should be named "discord_logs" for the exports to work properly.')
end
