...
ItemBack = function(name)
	local itemname = name
	local data = Config.ItemsOnback[itemname]
	local save = false
	if data and not onback[itemname] and currentWeapon and itemname ~= currentWeapon.name or currentWeapon == nil and data and not onback[itemname] then
...
AddEventHandler('ox_inventory:currentWeapon', function(cw)
	...
	currentWeapon = cw
	if currentWeapon ~= nil and joaat(currentWeapon.name) ~= joaat('WEAPON_UNARMED') then
		local ped = cache.ped
		ItemBack(lastweapon)
		for k,v in pairs(items) do
	...
	elseif currentWeapon and joaat(currentWeapon.name) == joaat('WEAPON_UNARMED') then
		ItemBack(lastweapon)
		lastweapon = currentWeapon.name
		currentWeapon = nil
...
ToggleItemBackSingle = function(name,bool)
	if onback[name] then
		SetEntityVisible(onback[name].entity, bool)
	else
		print('Items On Back: no item found', name, bool)
	end
end
...
AddEventHandler("n_snipppets:recheckItems", function()
	for itemName, item in pairs(onback) do
		if not DoesEntityExist(item.entity) then
			DeleteAttachments(item)
			ReqAndDelete(item.entity)
		end
	end
...
AddEventHandler("n_snippets:getModelsItemsOnBack", function(cb)
	local ItemsToSend = {}
	
	for item, data in pairs(Config.ItemsOnback) do ItemsToSend[joaat(data.model)] = true end
	for _item, _data in pairs(compodata) do ItemsToSend[joaat(_data.model)] = true end

	cb(ItemsToSend)
end)

function DespawnItemsOnBack()
	if next(onback) then
