...
CreateThread(function()
    ...
    if ESX.PlayerData.metadata.isTaxiDriver and ESX.PlayerData.metadata.isTaxiDriver == 1 then
...
-- AddEventHandler('esx:setPlayerData', function(key, metadata)
...
function setupSubItems()
    ...
    if PlayerData.metadata.isTaxiDriver and PlayerData.metadata.isTaxiDriver == 1 and not Config.MenuItems[5] then
        for i = 1, #Config.JobInteractions["taxi"] do
            Config.MenuItems[5] = {
                id = 'taxi_interactions',
                title = 'Taxi Cub',
                icon = 'taxi',
                items = Config.JobInteractions["taxi"]
            }
        end
    else
