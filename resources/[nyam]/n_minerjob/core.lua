...
if IsDuplicityVersion() then
    ...
    function Core.Notify(source, text, type, time)
    ...
    function Core.Notify(text, type, time)
        if not type then type = "primary" end
        if not time then time = 5000 end
        if Config.Options.notify == "default" then
            if Config.Options.framework == "qb" then
    ...
    function Core.Target.removeLocalEntity(data) 
        if Config.Options.target == "ox" then
    ...
    function Core.IsSpawnPointClear(coords, maxDistance)
        local vehs = EnumerateEntitiesWithinDistance(GetGamePool("CVehicle"), false, coords, maxDistance)

        return #vehs == 0
    end
    
    function Core.CreateBlip(data)
