	...
	RegisterCommand(Config.ClearEveryonesChatCommand, function(source, args, rawCommand)
		...
		if isAdmin(xPlayer) then
			...
			TriggerClientEvent('chat:addMessage', -1, {
				...
				args = { 'SYSTEM', 'The chat has been cleared!' }
			})
		else
			TriggerClientEvent('inventory:notification', source, 'ERROR', 'You are not allowed to use this command!', 8000, 'error')
		end
	end)
end
...
if Config.EnableStaffCommand then
	RegisterCommand(Config.StaffCommand, function(source, args, rawCommand)
		...
		if isAdmin(xPlayer) then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message staff"><i class="fas fa-shield-alt"></i><b><span style="color: #1ebc62">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
...
AddEventHandler('esx_identity:chat:notifystaff', function()
	showOnlyForAdmins(function(admins)
		TriggerClientEvent('chat:addMessage', admins, {
			template = '<div class="chat-message staffonly"><i class="fas fa-eye-slash"></i><b><span style="color: #1ebc62">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
			args = { 'ALERT', 'A NEW PLAYER JUST ARRIVED! PLEASE ASSIST HIM/HER WITH BASIC THINGS.' }
		})
	end)
end)
...
if Config.EnableStaffOnlyCommand then
	RegisterCommand(Config.StaffOnlyCommand, function(source, args, rawCommand)
		...
		if isAdmin(xPlayer) then
			...
			TriggerClientEvent('inventory:notification', source, 'ERROR', 'You are not a staff member!', 8000, 'error')
		end
	end)
end
	...
	RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
		...
		if canAdvertise then
			if bankMoney >= Config.AdvertisementPrice then
				...
				TriggerClientEvent('chat:addMessage', -1, {
					template = '<div class="chat-message advertisement"><i class="fas fa-ad"></i><b><span style="color: #81db44">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
					args = { playerName, message }
				})
				TriggerClientEvent('inventory:notification', source, "ADVERTISEMENT", "Advertisement successfully made for $"..Config.AdvertisementPrice, 10000, 'success')
				...
				TriggerClientEvent('inventory:notification', source, "ADVERTISEMENT", "You don't have enough money to make an advertisement", 10000, 'error')
			end
		else
			TriggerClientEvent('inventory:notification', source, "ADVERTISEMENT", "You can't advertise so quickly", 10000, 'error')
		end
	end)
end
...
if Config.EnableTwitchCommand then
	RegisterCommand(Config.TwitchCommand, function(source, args, rawCommand)
		...
		if twitch then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message twitch"><i class="fas fa-ad"></i><b><span style="color: #9c70de">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
				args = { playerName, message }
			})
		else
			TriggerClientEvent('inventory:notification', source, 'ERROR', 'You are not whitelisted Twitch Streamer!', 8000, 'error')
		end
	end)
end
...
if Config.EnableYoutubeCommand then
	RegisterCommand(Config.YoutubeCommand, function(source, args, rawCommand)
		...
		if youtube then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message youtube"><i class="fas fa-ad"></i><b><span style="color: #ff0000">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
				args = { playerName, message }
			})
		else
			TriggerClientEvent('inventory:notification', source, 'ERROR', 'You are not whitelisted YouTube Streamer!', 8000, 'error')
		end
	end)
end
...
if Config.EnableTwitterCommand then
	RegisterCommand(Config.TwitterCommand, function(source, args, rawCommand)
		...
		TriggerClientEvent('chat:addMessage', -1, {
			template = '<div class="chat-message twitter"><i class="fab fa-twitter"></i><b><span style="color: #2aa9e0">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
			args = { playerName, message }
		})
	end)
end
...
if Config.EnablePoliceCommand then
	RegisterCommand(Config.PoliceCommand, function(source, args, rawCommand)
		...
		if job == Config.PoliceJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message police"><i class="fas fa-shield"></i><b><span style="color: #4a6cfd">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
				args = { playerName, message }
			})
		else
			TriggerClientEvent('inventory:notification', source, 'ERROR', 'You are not a COP!', 8000, 'error')
		end
	end)
end
...
if Config.EnableAmbulanceCommand then
	RegisterCommand(Config.AmbulanceCommand, function(source, args, rawCommand)
		...
		if job == Config.AmbulanceJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message ambulance"><i class="fas fa-ambulance"></i><b><span style="color: #e3a71b">{0}:</span><span style="margin-top: 5px; font-weight: 300;">{1}</span></div>',
				args = { playerName, message }
			})
		else
			TriggerClientEvent('inventory:notification', source, 'ERROR', 'You are not an EMS!', 8000, 'error')
		end
	end)
end
...
function ooc(source, head, msg)
    local xPlayer = ESX.GetPlayerFromId(source)
    local message = msg
    local time = os.date(Config.DateFormat)
    playerName = xPlayer.getName()
    TriggerClientEvent('chat:ooc', -1, source, playerName, head, message, time)
end
exports('ooc', ooc)
