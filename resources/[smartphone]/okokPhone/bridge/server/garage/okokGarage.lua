local okokGarageConfig = exports['okokGarage']:getConfig()
...
local function getClosestGarage(source)
  ...
  for i, v in ipairs(okokGarageConfig.Garages) do
    if v.Type == "car" then
...
local function findGarageCoords(garageId)
  for _, garage in ipairs(okokGarageConfig.Garages) do
...
local function findImpoundCoords(impoundId)
  for _, impound in ipairs(okokGarageConfig.Impound) do
    if impound.name == impoundId then
      return impound.coords --[[ @as vector3 ]]
    end
  end
  return okokGarageConfig.Impound[1].coords
end
