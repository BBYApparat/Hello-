...
AddEventHandler("safecracking:loop", function(diff,from)
	loadSafeTexture()
	loadSafeAudio()
	difficultySetting = {}
	difficulty = diff
	...
	while cracking do
		...
		if GetDistanceBetweenCoords(mybasepos, GetEntityCoords(GetPlayerPed(-1))) > 1 and curLock <= difficulty then
			cracking = false
			TriggerEvent(from..":success", false)
		elseif GetDistanceBetweenCoords(mybasepos, GetEntityCoords(GetPlayerPed(-1))) > 1 or curLock > difficulty then
			cracking = false
		end
	...
	if curLock > difficulty then
		TriggerEvent(from..":success", true)
	end
	resetAnim()
