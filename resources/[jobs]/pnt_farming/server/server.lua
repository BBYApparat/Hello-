...
if Config.UseOxInventory then
    ...
    RegisterNetEvent('pnt_farming:giveItem', function(item)
        ...
        --if exports.inventory:canCarryItem(item, 1) then
    ...
    RegisterNetEvent('pnt_farming:processFruit', function(item)
        local xPlayer = ESX.GetPlayerFromId(source)
        if exports.inventory:canCarryItem(source, Config.ProcessZone.ItemBox) then 
    ...
    RegisterNetEvent('pnt_farming:giveItem', function(farmId)
        if not myItem[farmId] then return end

        local myItem = Config.Farms[farmId].ItemName
        local xPlayer = ESX.GetPlayerFromId(source)

        if xPlayer.canCarryItem(myItem, 1) then 
