Hi,
thank you for your purchase.
Please don't release my work any longer.

--------------------------------------------------

My contact: 
Cfx forum:			 https://forum.cfx.re/u/bzzzi/summary
Tebex:				 https://bzzz.tebex.io/
Discord:			  /PpAHBCMW97
--------------------------------------------------



Installation:
1) Insert folder "bzzz_kebab" to resources folder
2) Add to server.cfg
3) Restart server

You can then use the props as GTA props.


--------------------------------------------------
If you stream props in another resource, you must edit the fxmanifest.
Add this line:
data_file 'DLC_ITYP_REQUEST' 'stream/bzzz_electro_grill_kebab.ytyp'

Then the server must be restarted. 
YTYP loads properties of props.
--------------------------------------------------

I recommend that you don't spawn a props with a script.
Some scripts spawn static props.
If you can't write scripts, use ymap.