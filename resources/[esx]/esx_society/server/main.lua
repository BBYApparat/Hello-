...
local function registerSocietiesFromJobs(jobs, existingSocieties)
    local existingSocietiesMap = {}
    for _, society in ipairs(existingSocieties) do
        existingSocietiesMap[society.name] = true
    end

    for _, job in ipairs(jobs) do
        local societyName = job.name
        if not existingSocietiesMap[societyName] then
            local societyAccount = 'society_' .. job.name
            TriggerEvent('esx_society:registerSociety', societyName, job.label, societyAccount, societyAccount, societyAccount, {type = 'private'})
        end
    end
end
Citizen.CreateThread(function()
    ...
    MySQL.Async.fetchAll('SELECT * FROM jobs', {}, function(jobs)
        ...
        registerSocietiesFromJobs(jobs, RegisteredSocieties)
    end)
end)
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
    print("[DEBUG] esx_society:setJob called with identifier:", identifier, "job:", job, "grade:", grade, "type:", type)
    local xPlayer = ESX.GetPlayerFromId(source)
    local isBoss = xPlayer.job.grade_name == 'boss'

    if isBoss then
        local xTarget = ESX.GetPlayerFromIdentifier(identifier)
        print("[DEBUG] xTarget:", xTarget)

        if xTarget then
            xTarget.setJob(job, grade)
            print("[DEBUG] xTarget.setJob called")

            if type == 'hire' then
                xTarget.showNotification(_U('you_have_been_hired', job))
                TriggerEvent('society:jobOpened', job)
            ...
            elseif type == 'fire' then
                ...
                TriggerEvent('society:jobClosed', job)
            end
            ...
            MySQL.update('UPDATE users SET job = @job, job_grade = @job_grade WHERE identifier = @identifier', {
                ...
                print("[DEBUG] MySQL update rowsChanged:", rowsChanged)
                cb(rowsChanged > 0)
            end)
        end
    else
...
-- Event to handle job openings
RegisterNetEvent('society:jobOpened')
AddEventHandler('society:jobOpened', function(job)
    TriggerClientEvent('society:updateJobStatus', -1, job, true)
end)

-- Event to handle job closings
RegisterNetEvent('society:jobClosed')
AddEventHandler('society:jobClosed', function(job)
    TriggerClientEvent('society:updateJobStatus', -1, job, false)
end)
