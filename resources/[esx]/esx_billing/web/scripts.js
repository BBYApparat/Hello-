			...
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					if (invoices.status == 'paid') {
						...
						payment_status = `<button type="button" id="" class="btn btn-blue flex-grow-1 payInvoice" style="border-radius: 10px; flex-basis: 100%;" data-invoiceId="${invoices.id}" data-invoiceMoney="${invoices.invoice_value}" data-bs-dismiss="modal"><i class="fas fa-shopping-bag"></i> PAY ${invoices.invoice_value.toLocaleString()}$</button>`;
					}
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content myinvoices_modal-content">
											<div class="modal-body p-4">
												...
												<hr>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px;">Subtotal</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value - Math.round(invoices.invoice_value * (event.data.VAT/100))).toLocaleString()} $</span>
												</div>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px;">VAT (${event.data.VAT}%)</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value * (event.data.VAT/100)).toLocaleString()} $</span>
												</div>
												<br>
												<div class="d-flex justify-content-between">
...
$(document).on('click', ".payInvoice", function() {
	...
	$.post('https://esx_billing/action', JSON.stringify ({
		action: "payInvoice",
...
$(document).on('click', '#menuMyInvoices', function() {
	...
	$.post('http://esx_billing/action', JSON.stringify ({
...
$(document).on('click', '#sendInvoice', function() {
	...
	if(!invoice_value || !invoice_item || !player_id) {
		$.post('http://esx_billing/action', JSON.stringify({
			action: "missingInfo",
		}));
	} else {
