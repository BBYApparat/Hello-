...
RegisterNetEvent('jail_Blips:ReturnDelete')
AddEventHandler('jail_Blips:ReturnDelete', function(resource, shit)
...
AddEventHandler('playerSpawned', function()
	Citizen.Wait(3000)
	TriggerServerEvent('advanced_jail:LoadedIn')
end)
...
RegisterNetEvent('advanced_jail:JailStart')
AddEventHandler('advanced_jail:JailStart', function(timez)
	LocalPlayer.state:set('teleport', true, true)
	print('jail start', timez)
	TriggerEvent('policejob:uncuf')
	...
	Citizen.CreateThread(function()
		...
		SetBlockingOfNonTemporaryEvents(byped, true)
		...
		TriggerEvent('skinchanger:getSkin', function(skin)
			TriggerServerEvent('advanced_jail:UpdateClothes', skin)
			if skin.sex == 0 then
		...
		DoScreenFadeOut(1000)
		Citizen.Wait(1000)
		LoadJailCell(timez, false)
	end)
end)



RegisterNetEvent('advanced_jail:TakeBooze')
AddEventHandler('advanced_jail:TakeBooze', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('advanced_jail:ChangeLoc')
AddEventHandler('advanced_jail:ChangeLoc', function(newLoc)
	if inMenu.is then
...
RegisterNetEvent('advanced_jail:GoToJail')
AddEventHandler('advanced_jail:GoToJail', function(jtime, job, clothi)
	if clothi then
		TriggerEvent('skinchanger:getSkin', function(skin)
			if skin.sex == 0 then
				TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms.male)
...
function LoadJailCell(timu, firstTime)
	...
	ESX.TriggerServerCallback('advanced_jail:GrabInfoLoc', function(locnum)
		infoLoc = locnum
	end)
	...
	SetBlockingOfNonTemporaryEvents(byped2, true)
	...
	Citizen.CreateThread(function()
		...
		ESX.TriggerServerCallback('advanced_jail:GetCell', function(cell)
			print('cell data received', cell)
			if cell ~= 0 then
				TriggerServerEvent('advanced_jail:UpdateCell', cell)
				DoScreenFadeOut(1000)
				...
				SetEntityHeading(ped, Config.Cells[cell].SpawnLoc.Heading)
				jailCell = cell
				print('Getting jail time')
				ESX.TriggerServerCallback('advanced_jail:getjailtime', function(cb)
					print('jail time received', cb)
					LocalPlayer.state:set('teleport', false, true)
					...
					injail = true
					Citizen.Wait(1000)
					print(json.encode(difftime))
					TriggerServerEvent('advanced_jail:CheckSol', GetPlayerServerId(PlayerId()))
				end)
			end
		end)
	end)
...
RegisterNetEvent('advanced_jail:NotSol')
AddEventHandler('advanced_jail:NotSol', function()
	DoScreenFadeIn(500)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if breakout2 then
			...
			if ESX.PlayerData.job and Config.PoliceRoles[ESX.PlayerData.job.name] then
				...
				if dist <= Config.ShowItemDist then
					...
					if dist <= Config.ItemTextDist then
						...
						if IsControlJustReleased(1,38) then
							...
							inAnim.Freeze = true
							FreezeEntityPosition(ped, true)
							TriggerEvent('inventory:progress', Config.RetreiveTime *1000, Config.Sayings[13])
							Citizen.Wait(Config.RetreiveTime *1000)
							TriggerServerEvent('advanced_jail:RetrieveItems', itemzie)
							local removes = {}
							for i = 1, #blips, 1 do
			...
			if not using and not isDead then
				if dist <= Config.SeeWatchDist then
					...
					if dist <= Config.WatchDist then
						breakout2 = false
						breakout4 = true
						TriggerServerEvent('advanced_jail:UnBreak', GetPlayerServerId(PlayerId()))
					end
				else
			...
			if dist <= Config.SeeWorkDist and not using then
				...
				if dist <= Config.WorkText then
					...
					if IsControlJustReleased(0, 47) then
						...
						FreezeEntityPosition(ped, true)
						TriggerEvent('inventory:progress', Config.WorkoutLocs[workoutLoc].Locs[closestOut].Time *1000, Config.Sayings[123])
			...
			if dist <= Config.ShowerFullDist then
				...
				if dist <= Config.ShowerDist and not isDead then
					...
					if IsControlJustReleased(0, 47) then
						...
						TriggerEvent('inventory:progress', 18 *1000, Config.Sayings[118])
...
function StartWorkout(Loc)
	Citizen.CreateThread(function()
		...
		inAnim.Freeze = true
		FreezeEntityPosition(ped, true)
		TriggerEvent('inventory:progress', Config.WorkReadyTime *1000, Config.Sayings[114])
...
function StartShower()
	Citizen.CreateThread(function()
		...
		inAnim.Freeze = true
		FreezeEntityPosition(ped, true)
		TriggerEvent('inventory:progress', Config.GetReadyTime *1000, Config.Sayings[114])
...
function OpenBreakingMenu()
	...
	Citizen.CreateThread(function()
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fence_menu', {
			...
			if flip then
				ESX.TriggerServerCallback('advanced_jail:CheckItemB2', function(can)
					if can then
						menu.close()
						local hnum = 0
						...
						if hnum <= Config.RoomTools[data.current.value].Percent then
							...
							inAnim.Freeze = true
							FreezeEntityPosition(ped, true)
							TriggerEvent('inventory:progress', Config.RoomTools[data.current.value].Time *1000, Config.Sayings[96])
							...
							TriggerServerEvent('advanced_jail:UpdateBreaking')
							breakout3 = false
							breakout4 = false
							breakout = 0
							local removes = {}
							...
							TriggerServerEvent('advanced_jail:TakeItems2', Config.RoomTools[data.current.value].Item)
							using = true
							RequestAnimDict('mini@repair')
				...
				ESX.TriggerServerCallback('advanced_jail:CheckItemB2', function(can)
					if can then
						...
						if hnum <= Config.FenceTool[data.current.value].Percent then
							...
							inAnim.Freeze = true
							FreezeEntityPosition(ped, true)
							TriggerEvent('inventory:progress', Config.FenceTool[data.current.value].Time *1000, Config.Sayings[97])
...
function IEscaped()
	TriggerServerEvent('advanced_jail:UnJailPlayer', GetPlayerServerId(PlayerId()), false)
	TriggerServerEvent('advanced_jail:PoliceNotify')
	time = 0
	soltime = 0
	job = 0
	doneTasks = 0
...
function OpenWallMenu()
	...
	Citizen.CreateThread(function()
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wall_menu', {
			...
			ESX.TriggerServerCallback('advanced_jail:CheckItemB', function(can)
				if can then
					local hnum = 0
					local rannum = math.random(1,10)
					hnum = rannum
					menu.close()
					if hnum <= Config.RoomTools[data.current.value].Percent then
						TriggerServerEvent('advanced_jail:SuccessFul', GetPlayerServerId(PlayerId()), Config.RoomTools[data.current.value].Time *1000)
						using = true
						RequestAnimDict('mini@repair')
						...
						TriggerServerEvent('advanced_jail:TakeItems4', data.current.value)
						using = true
						RequestAnimDict('mini@repair')
...
function BreakOutStart()
	Citizen.CreateThread(function()
		...
		inAnim.Freeze = false
		FreezeEntityPosition(ped, true)
		TriggerEvent('inventory:progress', Config.CrawlTime *1000, Config.Sayings[161])
		...
		Citizen.Wait(1000)
		FreezeEntityPosition(ped, false)
		TriggerServerEvent('advanced_jail:UpdateBreak')
...
RegisterNetEvent('advanced_jail:UpBreaks')
AddEventHandler('advanced_jail:UpBreaks', function(amt, flip, time)
	local ped = PlayerPedId()
...
RegisterNetEvent('advanced_jail:UnBreak2')
AddEventHandler('advanced_jail:UnBreak2', function()
	Citizen.CreateThread(function()
		local ped = PlayerPedId()
		breakout3 = false
		breakout = 0
		...
		if Config.FailBreakToSol and Config.Solitary then
			TriggerServerEvent('advanced_jail:SendToSol', GetPlayerServerId(PlayerId()), Config.SolBreakTime, Config.Sayings[108])
		else
...
Citizen.CreateThread(function()
	while true do
		...
		elseif needsEat then
			...
			if IsControlJustReleased(0, 38) then
				TriggerEvent('inventory:progress', Config.EatTime *1000, Config.Sayings[139])
				...
				TriggerServerEvent('advanced_jail:Ate')
...
function OpenPoliceShitMenu()
	using = true
	ESX.TriggerServerCallback('advanced_jail:GetPlayerInCell', function(players)
		local ped = PlayerPedId()
		local element = {}
...
function OpenPoliceMenu2(name, theirID)
	Citizen.CreateThread(function()
		...
		inAnim.Freeze = true
		FreezeEntityPosition(ped, true)
		TriggerEvent('inventory:progress', Config.OpenCloseTime *1000, Config.Sayings[35])
		Citizen.Wait(Config.OpenCloseTime *1000)
	
		ESX.TriggerServerCallback('advanced_jail:GetChest2', function(cvalue)
			local valuesc = {}
			local element = {}
				...
				if data.current.value ~= 'none' then
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inv_amt', {
						...
						elseif amount > valuesc[numies].amt then
							...
							menu.close()
							TriggerServerEvent('advanced_jail:RemoveItem2', valuesc[numies].ite, amount, valuesc[numies].itemName, theirID)
						end
					end, function(data, menu)
						menu.close()
						TriggerEvent('inventory:progress', Config.OpenCloseTime *1000, Config.Sayings[36])
						Citizen.Wait(Config.OpenCloseTime *1000)
						FreezeEntityPosition(ped, false)
						inAnim.Dict = nil
						inAnim.Anim = nil
	...
	Citizen.CreateThread(function()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'make_menu2', {
			...
			if data.current.value == 'need' then
				...
				menu.close()
				ESX.TriggerServerCallback('advanced_jail:CheckItemMake', function(can)
					if can == 1 then
					...
					elseif can == 2 then
						...
						Citizen.CreateThread(function()
							using = true
							TriggerServerEvent('advanced_jail:TakeItems', itnum)
							local pedi = nil
							for i = 1, #peds, 1 do
								if peds[i].id == 'info' then
									pedi = peds[i].data
							...
							TriggerEvent('inventory:progress', Config.Crafts[itnum].Time *1000, Config.Sayings[85])
...
function TaskComplete()
	Citizen.CreateThread(function()
		...
		inAnim.Freeze = true
		FreezeEntityPosition(ped, true)
		TriggerEvent('inventory:progress', Config.JobOptions[job].Tasks[doneTasks].Time *1000, Config.Sayings[23])
		...
		if doneTasks >= taskMax then
			...
			TriggerServerEvent('advanced_jail:TaskComplete', job)
		else
			doneTasks = doneTasks + 1
			Notification(Config.Sayings[24])
		end
	
		TriggerServerEvent('advanced_jail:TaskComplete1', job)
				...
				if Config.TpBack then
					...
					if dist > Config.MaxTpDist then
						if Config.Sol4Run and Config.Solitary then
							TriggerServerEvent('advanced_jail:SendToSol', GetPlayerServerId(PlayerId()), Config.SolRunTime, Config.Sayings[109])
							Citizen.Wait(1000)
						else
							DoScreenFadeOut(1000)
							Citizen.Wait(1500)
...
function OpenAddChest()
	local ped = PlayerPedId()
	ESX.TriggerServerCallback('advanced_jail:GetInventory', function(inv)
		local invie = nil
		local element = {}
			...
			if data.current.value ~= 'none' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inv_amt2', {
					...
					elseif amount > invie[numies].count then
						...
						menu.close()
						TriggerServerEvent('advanced_jail:AddItem', invie[numies].name, amount, invie[numies].label, GetPlayerServerId(PlayerId()))
						OpenChest(false)
					end
				end, function(data, menu)
					menu.close()
...
function OpenItChest()
	local ped = PlayerPedId()
	ESX.TriggerServerCallback('advanced_jail:GetChest', function(cvalue)
		local valuesc = {}
		local element = {}
			...
			if data.current.value ~= 'none' then
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inv_amt', {
					...
					elseif amount > valuesc[numies].amt then
						...
						menu.close()
						TriggerServerEvent('advanced_jail:RemoveItem', valuesc[numies].ite, amount, valuesc[numies].itemName, GetPlayerServerId(PlayerId()))
						OpenChest(false)
					end
				end, function(data, menu)
					menu.close()
...
function OpenFood()
	Citizen.CreateThread(function()
		...
		inAnim.Freeze = true
		FreezeEntityPosition(ped, true)
		TriggerEvent('inventory:progress', Config.GrabFoodTime *1000, Config.Sayings[43])
...
function StartJob(jobie, trip)
	TriggerServerEvent('advanced_jail:SetJob', jobie, trip)
	local removes = {}
	for i = 1, #blips, 1 do
		if blips[i].id == 'task' then
			table.insert(removes, i)
...
RegisterNetEvent('advanced_jail:UnnJail')
AddEventHandler('advanced_jail:UnnJail', function(clothesi)
	Citizen.CreateThread(function()
		if not using then
			local ped = PlayerPedId()
			DoScreenFadeOut(1000)
...
function ResetLeave(twotwo)
	Citizen.CreateThread(function()
		if not using then
			TriggerEvent('advanced_jail:UnnJail', twotwo)
		else
			Citizen.Wait(3000)
			ResetLeave(twotwo)
		end
...
RegisterNetEvent('advanced_jail:SendSol')
AddEventHandler('advanced_jail:SendSol', function(soltimez, cell)
	Citizen.CreateThread(function()
		local ped = PlayerPedId()
		DoScreenFadeOut(1000)
		Citizen.Wait(1500)
...
RegisterNetEvent('advanced_jail:UnnSol')
AddEventHandler('advanced_jail:UnnSol', function()
	Citizen.CreateThread(function()
		local ped = PlayerPedId()
		using = true
		DoScreenFadeOut(1000)
...
RegisterNetEvent('advanced_jail:JailMenu')
AddEventHandler('advanced_jail:JailMenu', function()
	ESX.TriggerServerCallback('advanced_jail:CheckLockdown', function(elemento)
		local element = {}
		element = elemento

		local ped = PlayerPedId()
		local police = Config.PoliceRoles[ESX.PlayerData.job.name]
		inJailMenu = true
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'jail_main_menu', {
			...
			if data.current.value == 'jailplayer' then
				if police then
					...
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						inJailMenu = false
						TriggerEvent('advanced_jail:GetJailInfo', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
					else
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('advanced_jail:CheckID', function(can)
								if can then
									TriggerEvent('advanced_jail:GetJailInfo', amount, nil, nil)
								else
			...
			elseif data.current.value == 'unjail' then
				local isPolice = Config.PoliceRoles[ESX.PlayerData.job.name]

				if isPolice then
					local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						inJailMenu = false
						TriggerServerEvent('advanced_jail:UnJailPlayer', GetPlayerServerId(closestPlayer), true)
					else
						Notification(Config.Sayings[135])
					end
				else
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('advanced_jail:CheckID2', function(can)
								if can then
									ESX.UI.Menu.CloseAll()
									TriggerServerEvent('advanced_jail:UnJailPlayer', amount, true)
								else
									Notification(Config.Sayings[127])
								end
							end, amount)
			...
			elseif data.current.value == 'add' then
				if police then
					...
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						inJailMenu = false
						TriggerEvent('advanced_jail:AddJailTime', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
					else
						Notification(Config.Sayings[135])
					end
				else
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('advanced_jail:CheckID2', function(can)
								if can then
									TriggerEvent('advanced_jail:AddJailTime', amount, nil, nil)
								else
									Notification(Config.Sayings[127])
								end
							end, amount)
			...
			elseif data.current.value == 'remove' then
				if police then
					...
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						inJailMenu = false
						TriggerEvent('advanced_jail:RemoveJailTime', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
					else
						Notification(Config.Sayings[135])
					end
				else
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('advanced_jail:CheckID2', function(can)
								if can then
									TriggerEvent('advanced_jail:RemoveJailTime', amount, nil, nil)
								else
									Notification(Config.Sayings[127])
								end
							end, amount)
			...
			elseif data.current.value == 'solitary' then
				if Config.Solitary then
					if police then
						...
						if closestPlayer ~= -1 and closestDistance <= 3.0 then
							inJailMenu = false
							TriggerEvent('advanced_jail:SolitaryMenu', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
						else
							Notification(Config.Sayings[135])
						end
					else
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
							...
							if amount == nil or amount < 0 then
								...
								ESX.TriggerServerCallback('advanced_jail:CheckID2', function(can)
									if can then
										TriggerEvent('advanced_jail:SolitaryMenu', amount, nil, nil)
									else
										Notification(Config.Sayings[127])
									end
								end, amount)
			...
			elseif data.current.value == 'unsolitary' then
				if police then
					...
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
						inJailMenu = false
						TriggerServerEvent('advanced_jail:UnSol', GetPlayerServerId(GetPlayerPed(closestPlayer)))
					else
						Notification(Config.Sayings[135])
					end
				else
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('advanced_jail:CheckID2', function(can)
								if can then
									...
									TriggerServerEvent('advanced_jail:UnSol', amount)
								else
									Notification(Config.Sayings[127])
								end
							end, amount)
			...
			elseif data.current.value == 'lockdown' then
				if not switchie then
					switchie = true
					TriggerServerEvent('advanced_jail:SwitchLock')
					Wait(3000)
					switchie = false
				end
			...
			menu.close()
			inJailMenu = false
			if Config.PoliceRoles[ESX.PlayerData.job.name] then
				TriggerEvent('advanced_jail:ResetPMenu')
			end
		end)
	end)
end)

RegisterNetEvent('advanced_jail:GetJailInfo')
AddEventHandler('advanced_jail:GetJailInfo', function(id, time, reason)
	local ped = PlayerPedId()
	local element = {
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'jail_menu', {
		...
		if data.current.value == 'id' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
				...
				if amount == nil or amount < 0 then
					...
					ESX.TriggerServerCallback('advanced_jail:CheckID', function(can)
						if can then
							TriggerEvent('advanced_jail:GetJailInfo', amount, time, reason)
						else
							Notification(Config.Sayings[127])
						end
					end, amount)
				...
				TriggerEvent('advanced_jail:GetJailInfo', id, time, reason)
			end)
		elseif data.current.value == 'time' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'time_amt', {
				...
				elseif amount < Config.MinJail then
					Notification(Config.Sayings[179])
			    else
					TriggerEvent('advanced_jail:GetJailInfo', id, amount, reason)
				end
			end, function(data, menu)
				TriggerEvent('advanced_jail:GetJailInfo', id, time, reason)
			end)
		elseif data.current.value == 'reason' then
			...
			if reason ~= nil and id ~= nil and time ~= nil then
				menu.close()
				TriggerServerEvent('advanced_jail:sendToJail', id, time *60, reason, Config)
			else
				if reason == nil then
...
RegisterNetEvent('advanced_jail:AddJailTime')
AddEventHandler('advanced_jail:AddJailTime', function(id, time, reason)
	local ped = PlayerPedId()
	local element = {
		[1] = {label = Config.Sayings[56]..id, value = 'id'},
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'jail_menu5', {
		...
		if data.current.value == 'id' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt2', {
				...
				if amount == nil or amount < 0 then
					...
					ESX.TriggerServerCallback('advanced_jail:CheckID2', function(yurrrr)
						if yurrrr then
							menu.close()
							TriggerEvent('advanced_jail:AddJailTime', amount, time, reason)
						else
				...
				menu.close()
				TriggerEvent('advanced_jail:AddJailTime', id, time, reason)
			end)
		elseif data.current.value == 'time' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'time_amt2', {
				...
				elseif amount > Config.MaxJail then
					...
					menu.close()
					TriggerEvent('advanced_jail:AddJailTime', id, amount, reason)
				end
			end, function(data, menu)
				menu.close()
				TriggerEvent('advanced_jail:AddJailTime', id, time, reason)
		...
		elseif data.current.value == 'reason' then
			...
			if reason ~= nil and id ~= nil and time ~= nil then
				menu.close()
				TriggerServerEvent('advanced_jail:AddSomeTime', id, time *60, reason)
			end
		end
	end, function(data, menu)
		menu.close()
...
RegisterNetEvent('advanced_jail:RemoveJailTime')
AddEventHandler('advanced_jail:RemoveJailTime', function(id, time, reason)
	local ped = PlayerPedId()
	local element = {
		[1] = {label = Config.Sayings[56]..id, value = 'id'},
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'jail_menu6', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt2', {
				...
				if amount == nil or amount < 0 then
					...
					ESX.TriggerServerCallback('advanced_jail:CheckID2', function(yurrrr)
						if yurrrr then
							menu.close()
							TriggerEvent('advanced_jail:RemoveJailTime', amount, time, reason)
						else
							Notification(Config.Sayings[103])
						end
					end, amount)
				...
				menu.close()
				TriggerEvent('advanced_jail:RemoveJailTime', id, time, reason)
			end)
		elseif data.current.value == 'time' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'time_amt2', {
				title = (Config.Sayings[63])
				...
				elseif amount > Config.MaxJail then
					...
					menu.close()
					TriggerEvent('advanced_jail:RemoveJailTime', id, amount, reason)
				end
			end, function(data, menu)
				menu.close()
				TriggerEvent('advanced_jail:RemoveJailTime', id, time, reason)
		...
		elseif data.current.value == 'reason' then
			...
			if reason ~= nil and id ~= nil and time ~= nil then
				menu.close()
				TriggerServerEvent('advanced_jail:RemoveSomeTime', id, time *60, reason)
			end
		end
	end, function(data, menu)
		menu.close()
...
RegisterNetEvent('advanced_jail:SolitaryMenu')
AddEventHandler('advanced_jail:SolitaryMenu', function(id, time, reason)
	local ped = PlayerPedId()
	local element = {
		[1] = {label = Config.Sayings[56]..id, value = 'id'},
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'jail_menu6', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt2', {
				...
				if amount == nil or amount < 0 then
					...
					ESX.TriggerServerCallback('advanced_jail:CheckID2', function(yurrrr)
						if yurrrr then
							menu.close()
							TriggerEvent('advanced_jail:SolitaryMenu', amount, time, reason)
						else
							Notification(Config.Sayings[103])
						end
					end, amount)
				...
				menu.close()
				TriggerEvent('advanced_jail:SolitaryMenu', id, time, reason)
			end)
		elseif data.current.value == 'time' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'time_amt2', {
				title = (Config.Sayings[63])
				...
				elseif amount > Config.MaxJail then
					...
					menu.close()
					TriggerEvent('advanced_jail:SolitaryMenu', id, amount, reason)
				end
			end, function(data, menu)
				menu.close()
				TriggerEvent('advanced_jail:SolitaryMenu', id, time, reason)
		...
		elseif data.current.value == 'reason' then
			...
			if reason ~= nil and id ~= nil and time ~= nil then
				menu.close()
				TriggerServerEvent('advanced_jail:SendToSol', id, time, reason)
			end
		end
	end, function(data, menu)
		menu.close()
...
RegisterNetEvent('advanced_jail:AddItUp')
AddEventHandler('advanced_jail:AddItUp', function(timy)
	if time > 0 then
		time = time + timy
...
RegisterNetEvent('advanced_jail:Removeit')
AddEventHandler('advanced_jail:Removeit', function(timy)
	if time > 0 then
		time = time - timy
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reason_menu', {
		...
		if data.current.value == 'confirm' then
			if reason ~= nil then
				...
				elseif menuz == 5 then
					menu.close()
					TriggerServerEvent('advanced_jail:Send2Prisoners', reason)
				end
			else
		...
		elseif menuz == 2 then
			TriggerEvent('advanced_jail:AddJailTime', id, time, reason)
		elseif menuz == 3 then
			TriggerEvent('advanced_jail:RemoveSomeTime', id, time, reason)
		elseif menuz == 4 then
			TriggerEvent('advanced_jail:SolitaryMenu', id, time, reason)
		elseif menuz == 5 then
			TriggerEvent('advanced_jail:JailMenu')
...
AddEventHandler('esx:onPlayerDeath', function()
	isDead = true
	TriggerServerEvent('advanced_jail:PlayerDie', true)
end)
	...
	Citizen.CreateThread(function()
		...
		if isDead and time > 0 then
			...
			if Config.Hospital then
				...
				TriggerServerEvent('advanced_jail:UnSee', GetPlayerServerId(PlayerId()))
				...
				Citizen.Wait(1000)
				TriggerEvent('inventory:progress', Config.CheckUpTime *1000, Config.Sayings[70])
				...
				ClearPedTasksImmediately(ped)
				ClearPedTasksImmediately(byped)
				TriggerServerEvent('advanced_jail:See', GetPlayerServerId(PlayerId()))
			...
			TriggerServerEvent('advanced_jail:PlayerDie', false)
			isDead = false
			using = false
			DoScreenFadeIn(1000)
			Citizen.Wait(1500)
...
RegisterNetEvent('advanced_jail:ShankPull')
AddEventHandler('advanced_jail:ShankPull', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('advanced_jail:GiveShankie')
AddEventHandler('advanced_jail:GiveShankie', function()
	local ped = PlayerPedId()
	local shankie = GetHashKey(Config.ShankWeapon)
	GiveWeaponToPed(ped, shankie, 100, false, true)
end)

RegisterNetEvent('advanced_jail:PoliceWarning')
AddEventHandler('advanced_jail:PoliceWarning', function(name)
	local goodo = false
	if ESX.PlayerData.job and Config.PoliceRoles[ESX.PlayerData.job.name] then
		goodo = true
	end
...
RegisterNetEvent('advanced_jail:SendNotif2')
AddEventHandler('advanced_jail:SendNotif2', function(msg)
	if time > 0 then
		Notification(msg)
	end
end)

RegisterNetEvent('advanced_jail:SendNotif')
AddEventHandler('advanced_jail:SendNotif', function(msg)
	Notification(msg)
end)

RegisterNetEvent('advanced_jail:CountWarn')
AddEventHandler('advanced_jail:CountWarn', function(time)
	if time > 0 then
		Notification(time..Config.Sayings[149])
	end
end)

RegisterNetEvent('advanced_jail:TurnOffLock')
AddEventHandler('advanced_jail:TurnOffLock', function()
	lockieDown = false
	if inJailMenu then
		TriggerEvent('advanced_jail:JailMenu')
	end
...
RegisterNetEvent('advanced_jail:CountFinish')
AddEventHandler('advanced_jail:CountFinish', function()
	if inJailMenu then
		TriggerEvent('advanced_jail:JailMenu')
	end
	lockieDown = true
						...
						if soltime == 0 and breakout == 0 then
							...
							if dist > Config.LockDownDist then
								if Config.Solitary and Config.Sol4Lock then
									using = true
									TriggerServerEvent('advanced_jail:SendToSol', GetPlayerServerId(PlayerId()), Config.SolLockTime, Config.Sayings[150])
								else
									using = true
									SetEntityCoords(ped, Config.Cells[jailCell].SpawnLoc.Loc.x, Config.Cells[jailCell].SpawnLoc.Loc.y, Config.Cells[jailCell].SpawnLoc.Loc.z - 1, false, false, false, false)
									SetEntityHeading(ped, Config.Cells[jailCell].SpawnLoc.Heading)
...
AddEventHandler('radialmenu:jail', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		inJailMenu = false
		TriggerEvent('advanced_jail:GetJailInfo', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
...
AddEventHandler('radialmenu:unjail', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		inJailMenu = false
		TriggerServerEvent('advanced_jail:UnJailPlayer', GetPlayerServerId(closestPlayer), true)
...
AddEventHandler('radialmenu:addtime', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		inJailMenu = false
		TriggerEvent('advanced_jail:AddJailTime', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
...
AddEventHandler('radialmenu:removetime', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		inJailMenu = false
		TriggerEvent('advanced_jail:RemoveJailTime', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
...
AddEventHandler('radialmenu:solitary', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		inJailMenu = false
		TriggerEvent('advanced_jail:SolitaryMenu', GetPlayerServerId(GetPlayerPed(closestPlayer)), nil, nil)
...
AddEventHandler('radialmenu:unsolitary', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		inJailMenu = false
		TriggerServerEvent('advanced_jail:UnSol', GetPlayerServerId(GetPlayerPed(closestPlayer)))
...
AddEventHandler('radialmenu:lockdown', function()
	if not switchie then
		switchie = true
		TriggerServerEvent('advanced_jail:SwitchLock')
		Wait(3000)
...
AddEventHandler('radialmenu:mssg', function()
	inJailMenu = false
	OpenReason(nil, nil, nil, 5)
end)
