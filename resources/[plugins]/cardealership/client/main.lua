...
local created_peds = {}
local target_zones = {} -- Track created zones for cleanup
...
local GeneratePlate = function(type)
	...
	local doBreak = false
	local plateSpace = Config.PlateUseSpace
	local plateLet = Config.PlateLetters
	local plateNum = Config.PlateNumbers
	while true do
		Wait(2)
		math.randomseed(GetGameTimer())
		if plateSpace then
...
RegisterNUICallback('BuyVehicle', function(data, cb)
    ...
    if not GetAvailableVehicleSpawnPoint(spawn.coords) then
        ...
        if v.preview and v.preview.coords and v.preview.type then
            ...
            if v.blip then
                ...
                if blipData.enabled then
                    local blip = AddBlipForCoord(v.preview.coords.x, v.preview.coords.y, v.preview.coords.z)
                ...
                if coords.x and coords.y and coords.z and targetData and targetData.type then
                    if targetData.type == "boxzone" then
                        ...
                        local zoneId = exports.ox_target:addBoxZone({
                            ...
                            rotation = v.preview.heading or 0.0,
                            debug = false,
                            options = {
                                {
                                    name = "open_vehicleshop_" .. k,
                                    event = "vehicleshop:openStore",
                                    icon = targetData.icon or "fas fa-car",
                                    label = "Open " .. v.label,
                                    distance = v.preview.distance or 2.0
                                }
                            }
                        })
                        target_zones[k] = zoneId
                    elseif targetData.type == "entityzone" then
                        if targetData and targetData.model then
                            local model = GetHashKey(targetData.model)
                            loadModel(model)
                            local created_ped = CreatePed(1, model, coords.x, coords.y, coords.z - 0.9, v.preview.heading or 0.0, false, false)
                            ...
                            -- ox_target syntax for entities
                            exports.ox_target:addLocalEntity(created_ped, {
                                {
                                    name = "open_vehicleshop_ped_" .. k,
                                    event = "vehicleshop:openStore",
                                    icon = targetData.icon or "fas fa-car",
                                    label = "Open " .. v.label,
                                    distance = v.preview.distance or 2.0
...
if Config.Toggles.useGangs then
    RegisterNetEvent(Config.Events.setGang, function(gang)
        setGangData(gang)
    end)
end
...
removeTargetZones = function()
    for k, zoneId in pairs(target_zones) do
