...
-- Admin command to get keys for any vehicle (lock/unlock only)
if ESX then
    ESX.RegisterCommand('getadminkeys', 'admin', function(xPlayer, args)
        ...
        -- Just give keys to admin without taking ownership
        TriggerClientEvent('carLock:setVehicleKey', source, plate)
        
    end, true, {help = 'Get admin keys for lock/unlock access to any vehicle'})
else
    -- Fallback for non-ESX servers using standard RegisterCommand
    RegisterCommand('getadminkeys', function(source, args, rawCommand)
        -- Simple admin check - you can modify this based on your admin system
        if not IsPlayerAceAllowed(source, 'command.getadminkeys') then
            return
        end
...
-- ========== EXPORTS FOR GARAGE INTEGRATION ==========

-- Export for automatic key assignment when vehicle is spawned from garage
exports('giveKeys', function(playerId, plate)
    ...
    vehicleStates[plate] = { locked = false, owner = playerId }
    
    -- Add vehicle to the player's owned list
    playerVehicles[playerId] = playerVehicles[playerId] or {}
    table.insert(playerVehicles[playerId], plate)
    
    -- Give keys to client
    TriggerClientEvent('carLock:setVehicleKey', playerId, plate)
    return true
end)

-- Export for removing keys (useful for vehicle sales, etc.)
exports('removeKeys', function(playerId, plate)
    if not playerId or not plate then return false end
    
    -- Remove from vehicle states
    if vehicleStates[plate] and vehicleStates[plate].owner == playerId then
        vehicleStates[plate] = nil
    end
    
    -- Remove from player's vehicle list
    if playerVehicles[playerId] then
        for i, p in ipairs(playerVehicles[playerId]) do
            if p == plate then
                table.remove(playerVehicles[playerId], i)
                break
            end
        end
    end
    
    -- Remove keys from client
    TriggerClientEvent('carLock:removeVehicleKey', playerId, plate)
    return true
end)
