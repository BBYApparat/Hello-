...
function GetPlayerGender()
    ...
    -- ESX uses 'm' and 'f' for gender
    if playerData.sex == 'f' or playerData.sex == 'female' then
        gender = locale('female')
    elseif playerData.gender == 1 then -- Some ESX versions use 0/1
        gender = locale('female')
    end
...
function IsOnDuty()
    if Config.OnDutyOnly then
        ...
        if playerData.job then
            ...
            if playerData.job.duty ~= nil then
                return playerData.job.duty
            end
            -- If no duty system, consider always on duty for emergency services
            local emergencyJobs = {'police', 'ambulance', 'fire', 'sheriff', 'state'}
            if lib.table.contains(emergencyJobs, playerData.job.name) then
                return true
            end
        end
        return false
...
local function HasPhone()
    for _, item in ipairs(Config.PhoneItems) do
        -- ESX item check - multiple methods for compatibility
...
-- Alternative HasPhone function using server callback for better compatibility
local function HasPhoneServer()
    local hasPhone = false
    ESX.TriggerServerCallback('ps-dispatch:server:hasItem', function(result)
        hasPhone = result
    end, Config.PhoneItems)
    
    -- Wait for callback response
    local timeout = 0
    while hasPhone == false and timeout < 50 do
        Wait(10)
        timeout = timeout + 1
    end
...
function PhoneAnimation()
    ...
    if not IsEntityPlayingAnim(ped, "cellphone@in_car@ds", "cellphone_call_listen_base", 3) then
        TaskPlayAnim(ped, "cellphone@in_car@ds", "cellphone_call_listen_base", 3.0, 3.0, -1, 50, 0, false, false, false)
    end

    Wait(2500)
    StopEntityAnim(ped, "cellphone_call_listen_base", "cellphone@in_car@ds", 3)
end
...
function IsCallAllowed(message)
    ...
    if Config.PhoneRequired and not HasPhone() then 
        ESX.ShowNotification('You need a communications device for this.', 'error', 5000) 
        return false 
    end
...
-- Additional ESX-specific utility functions

-- Get player name for ESX
...
-- Get player job for ESX
function GetPlayerJob()
    local playerData = ESX.GetPlayerData()
    if playerData.job then
        return playerData.job.name, playerData.job.label, playerData.job.grade
    end
    return "unemployed", "Unemployed", 0
end

-- Check if player has specific item (ESX compatible)
function HasItem(itemName, amount)
    amount = amount or 1
    
    if exports.ox_inventory then
        return exports.ox_inventory:GetItemCount(itemName) >= amount
    else
        -- Server callback method for standard ESX
        local hasItem = false
        ESX.TriggerServerCallback('ps-dispatch:server:hasItem', function(result)
            hasItem = result
        end, {itemName}, amount)
        ...
        local timeout = 0
        while hasItem == false and timeout < 50 do
            Wait(10)
            timeout = timeout + 1
        end
...
-- Check if player is in vehicle for ESX
function IsInVehicle()
    local ped = PlayerPedId()
    return IsPedInAnyVehicle(ped, false)
end

-- Get vehicle player is in for ESX
