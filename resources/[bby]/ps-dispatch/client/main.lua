...
-- Check if player is on duty (ESX compatible)
local function IsOnDuty()
    if not PlayerData.job then return false end
    
    -- Check if job has duty system
    if Config.JobsWithDuty and Config.JobsWithDuty[PlayerData.job.name] then
        return PlayerData.job.duty or false
    end
    
    -- If no duty system, consider always on duty
    return true
end
...
-- Add support for duty system if available
RegisterNetEvent("esx:toggleDuty", function(duty)
    if PlayerData.job then
        PlayerData.job.duty = duty
    end
end)

-- Add support for ox_core if used instead of ESX
    ...
    RegisterNetEvent('ox:playerLoaded', function()
        ...
        if player then
            PlayerData = {
                ...
                metadata = {
                    callsign = player.metadata and player.metadata.callsign or "Unknown"
                },
                identifier = player.charid,
                job = {
                    type = player.getGroup(),
                    name = player.getGroup(),
                    label = player.getGroup(),
                    grade = player.getGrade(),
                    grade_name = player.getGrade(),
                    grade_label = player.getGrade()
                },
            }
            setupDispatch()
            createZones()
