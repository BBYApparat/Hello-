...
RegisterNetEvent("s4-realisticdisease:hitRecieveVuran")
AddEventHandler("s4-realisticdisease:hitRecieveVuran", function(bone, injures)
    local cachet = {}
    for k,v in pairs(CurrentInjure.injures) do
        table.insert(cachet, k)
    end
...
AddEventHandler("s4-realisticdisease:dmg", function(victim, damage, victim_ped, LastDamagedBone)
...
AddEventHandler('gameEventTriggered', function(event, data)
    ...
    if event == 'CEventNetworkEntityDamage' then
        ...
        if victimDied and NetworkGetPlayerIndexFromPed(victim) == PlayerId() then
            ...
            TriggerEvent("s4-realisticdisease:dmg", victimid, damage, victim, LastDamagedBone)
        end
    end
end)