...
-- Knockout system
RegisterNetEvent("ars_ambulancejob:helpKnockedPlayer", function(targetServerId)
    local src = source
    if not src or src < 1 or not targetServerId or targetServerId < 1 then return end
    ...
    if #(GetEntityCoords(sourcePed) - GetEntityCoords(targetPed)) > 3.0 then
        return print(src .. ' possible modder - too far from knocked out player')
    end
    
    -- Trigger recovery on target
    TriggerClientEvent('ars_ambulancejob:knockoutHelped', targetServerId)
    
    -- Optional: Give both players some feedback
    TriggerClientEvent('ars_ambulancejob:showNotification', src, 'helped_player')
    TriggerClientEvent('ars_ambulancejob:showNotification', targetServerId, 'got_helped')
end)
