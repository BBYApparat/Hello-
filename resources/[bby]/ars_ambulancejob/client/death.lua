    ...
    CreateThread(function()
        while player.isDead do
            ...
            if not player.gettingRevived and not player.respawning then
                ...
                local remainingTime = math.floor((time / 1000) - elapsedSeconds)
                ...
                if GetGameTimer() - deathTime >= time then
                    -- Enable respawn in UI
                    SendNUIMessage({
                        action = 'enableRespawn'
                    })
                end
            end
...
-- NUI Callbacks for death screen
RegisterNUICallback('callForHelp', function(data, cb)
    createDistressCall()
    cb('ok')
end)
...
RegisterNUICallback('respawn', function(data, cb)
    local confermation = lib.alertDialog({
        header = 'Respawn',
        content = 'Are you sure you want to respawn? This action cannot be undone.',
        centered = true,
        cancel = true
    })
...
RegisterNUICallback('timerComplete', function(data, cb)
    -- Timer has completed, respawn is now available
    cb('ok')
end)
