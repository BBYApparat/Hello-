...
function RemoveItem(source, item, count, slot, metadata)
    if RemoveItemData[Utils.InventoryName] then
        return RemoveItemData[Utils.InventoryName](source, item, count, slot, metadata)
    end
end
...
RemoveItemData = {
    ...
    ["gfx-inventory"] = function(source, item, count, slot)
        return exports["gfx-inventory"]:RemoveItem(source, "inventory", item, count)
    end,
    ["ox_inventory"] = function(source, item, count, slot, metadata)
...
function string.split(inputString)
    ...
    for i = 1, string.len(inputString) do
        table.insert(characters, string.sub(inputString, i, i))
    end
    return characters
end
