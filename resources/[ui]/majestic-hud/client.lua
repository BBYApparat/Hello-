    ...
    CreateThread(function()
        while veh == cache.vehicle do
            ...
            if speed >= stressSpeed then
                framework:triggerStressGain(math.random(1, 3))
            end
            Wait(10000)
        end
    end)
...
CreateThread(function()
    ...
    while true do        
        ...
        if Config.framework == 'qb' then
            ...
            isDead = isDead or playerData.metadata["inlaststand"] or playerData.metadata["isdead"] or false
        end
...
-- Framework-specific events are now handled in framework modules

AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() ~= resourceName then return end
    Wait(1000)
    -- Framework will handle initialization
end)
